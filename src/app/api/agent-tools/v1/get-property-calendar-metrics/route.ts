...
import { getPropertyCalendarMetrics } from "@/lib/agent-tools/service";
import { enforceDateWindow, handleToolError, parseQuery, requireScopedSession, toObjectId } from "@/lib/agent-tools/utils";
import { propertyCalendarMetricsSchema } from "@/lib/agent-tools/schemas";
import { toolLogger as log } from "@/lib/agent-tools/logger";

const EP = "GET /get-property-calendar-metrics";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const query = parseQuery(propertyCalendarMetricsSchema, searchParams);
    ...
    log.serviceCall(EP, "getPropertyCalendarMetrics");
    const data = await getPropertyCalendarMetrics(
      orgId,
      toObjectId(query.listingId),
      query.dateFrom,
      query.dateTo
    );
    log.serviceResult(EP, "getPropertyCalendarMetrics", {
    ...
    log.resSuccess(EP, 200, ms, { occupancyPct: data.occupancyPct });
