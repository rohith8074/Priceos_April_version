...
export async function GET(request: Request) {
  ...
  try {
    ...
    const query = parseQuery(guestSummarySchema, searchParams);
    ...
    log.serviceCall(EP, "getGuestSummary");
    const data = await getGuestSummary(orgId, toObjectId(query.listingId), query.dateFrom, query.dateTo);
    log.serviceResult(EP, "getGuestSummary", {
      cached: data.cached,
      stale: data.stale,
    ...
    log.resSuccess(EP, 200, ms, { cached: data.cached, stale: data.stale });
