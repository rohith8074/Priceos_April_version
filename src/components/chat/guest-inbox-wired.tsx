...
function differenceInDaysSafe(dateTo: string, dateFrom: string): number {
  if (!dateTo || !dateFrom) return 0;
  try {
    const d1 = new Date(dateTo);
    const d2 = new Date(dateFrom);
          ...
          {conv.checkIn && conv.checkOut && (
            <div className="flex items-center gap-1 text-text-muted truncate">
              ...
              <span className="text-[9px]">
                {formatSafeDate(conv.checkIn, "d MMM")} – {formatSafeDate(conv.checkOut, "d MMM")}
              </span>
            </div>
          )}
        </div>
...
export function GuestInboxWired({ orgId, properties }: { orgId: string; properties: PropertyWithMetrics[] }) {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      // Create a lookup map for properties
      const propertyMap: Record<string, PropertyWithMetrics> = {};
      ...
      backendConvs.forEach((c, i) => {
        ...
        if (property) {
          ...
          // Fallback to first property if listingId doesn't match exactly
          merged.push(mapConversation(c, properties[0], i));
        }
      });
                ...
                {conv.checkIn && conv.checkOut && (
                  <>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[11px] text-text-secondary">{formatSafeDate(conv.checkIn, "d MMM")} – {formatSafeDate(conv.checkOut, "d MMM yyyy")}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Clock className="h-3 w-3 text-amber shrink-0" />
                      <span className="text-[11px] text-text-secondary">{differenceInDaysSafe(conv.checkOut, conv.checkIn)} nights</span>
                    </div>
                  </>
                )}
              </div>
